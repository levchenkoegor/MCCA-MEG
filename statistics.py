...
#X_gr1 = np.load(proj_root / 'test' / 'Gr1_data.npy')
#X_gr2 = np.load(proj_root / 'test' / 'Gr2_data.npy')

X_both_sep = np.load(str(proj_root / 'test' / 'X_both_sep.npy'), allow_pickle=True).item()
X_both_cat = np.load(str(proj_root / 'test' / 'X_both_cat.npy'), allow_pickle=True).item()

isc_results_pseudo_groups = dict()
[W, ISC_overall] = train_cca(X_both_cat)
for i in range(0, 200):
    pseudo_gr1_inds = random.sample(range(0, 40), 20)
    pseudo_gr2_inds = [ind for ind in list(range(0, 40)) if ind not in pseudo_gr1_inds]

    X_pseudo_groups = dict(Gr1=X_both_cat['Gr1_2'][pseudo_gr1_inds, :, :], Gr2=X_both_cat['Gr1_2'][pseudo_gr2_inds, :, :])
...
np.save(str(proj_root / 'test' / 'isc_results_pseudo_groups'), isc_results_pseudo_groups)
